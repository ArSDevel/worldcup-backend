 ...
 * =================================
 * Este servidor funciona como puente seguro:
 *
...
const MAX_LIVE_SECONDS = 150 * 60; // 2h 30min
...
async function fetchFromSportradar() {
  ...
  const matches = sportEvents.map((event) => {
    ...
    return {
      id: String(event.id || event.sport_event?.id || `${home?.name}-${away?.name}-${kickoffUnix}`),
      home: home?.name || "TBD",
      away: away?.name || "TBD",
      homeScore,
      awayScore,
...
async function fetchFromSportMonks() {
  ...
  const matches = fixtures.map((fixture) => {
    ...
    return {
      id: String(fixture.id || `${home?.name}-${away?.name}-${kickoffUnix}`),
      home: home?.name || "TBD",
      away: away?.name || "TBD",
      homeScore,
      awayScore,
